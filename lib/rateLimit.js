...
// Check rate limit for a client (simplified API)
export async function checkRateLimit(clientId, endpoint = 'general') {
  const config = RATE_LIMIT_CONFIGS[endpoint] || RATE_LIMIT_CONFIGS.general;
  const key = `${endpoint}:${clientId}`;
  ...
  return {
    ...
    retryAfter: limitCheck.allowed ? 0 : Math.ceil((limitCheck.resetTime - now) / 1000)
  };
}
